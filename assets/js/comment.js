    ...
    */
    $('#csubmit').click(function () {
        ...
        if($("#cnumber").val().length == 0 || $("#cuname").val().length == 0 || $("#cmsg").val().length == 0){
			...
			$.get('/getComment',{username: $("#cuname").val(), number:$("#cnumber").val(),comment: $("#cmsg").val()}, function(commentsdb){
				if(commentsdb)
					console.log("getcomment done");
					res.redirect('/commentspage', commentsdb);
			})
		}
    });
